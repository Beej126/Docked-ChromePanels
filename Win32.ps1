...
public class Win32 {
    ...
    public static IntPtr FindWindowByTitle(string title, string newWindowTitle = null) {
      IntPtr hwnd = new IntPtr(0);
      var tries = 0;
      while(1==1) {
        hwnd = FindWindow(null, title);
        if ((int)hwnd > 0) break;
        if (++tries > 6) return hwnd;
        System.Threading.Thread.Sleep(500);
      }
      if (newWindowTitle != null) SetWindowText(hwnd, newWindowTitle);
      return hwnd;
    }
    ...
    static extern int Win32GetWindowText(IntPtr hWnd, StringBuilder lpString, int nMaxCount);
    ...
    public static string GetWindowText(IntPtr hWnd)
    {
        ...
        Win32GetWindowText(hWnd, sb, sb.Capacity);
        return sb.ToString();
    }
